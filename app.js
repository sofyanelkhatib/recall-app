...
function parseImportedJson(rawText) {
  ...
  return {
    ...
    cards: cards.slice(0, CONFIG.MAX_CARDS_PER_SET),
  };
}
...
function initCreateView() {
  ...
  async function handleApkg(file) {
    ...
    try {
      ...
      const capped = result.cards.length > CONFIG.MAX_CARDS_PER_SET;
      ...
      document.getElementById("apkg-summary").innerHTML = `
        ...
        ${capped ? `<div class="hint" style="color:var(--amber)">Only the first ${CONFIG.MAX_CARDS_PER_SET} will be imported.</div>` : ""}
      `;
  ...
  document.getElementById("apkg-import-btn").addEventListener("click", async () => {
    ...
    const cards = pendingApkg.cards.slice(0, CONFIG.MAX_CARDS_PER_SET);
...
async function startStudySession(deckId) {
  ...
  // Study due cards first; if nothing is due yet, study the whole deck (fresh set / early review)
...
function renderStudyCard() {
  ...
  document.getElementById("study-set-title").textContent = s.deckTitle;
...
async function finishStudySession(early = false) {
  ...
  const statsHtml = `
    <div class="stat-block"><div class="stat-value">${total}</div><div class="stat-label">Reviewed</div></div>
